...
export class ScriptDirector {
    ...
    for (const episodeNumber of range) {
      ...
      if (!draft) {
        ...
        const prompt = [
          ...
          '同一证据、照片、电话或动作在本集只能首次发现一次；在全剧后续只能写核验、转交、追查或产生的新后果。前面集数已经完整演过的场景不得换人物、地点或措辞后重演。同一道具动作链只完整演一次：例如已经写过“打开抽屉—拿起照片—看完放回”，后面不能换个人再次从打开同一抽屉、查看同一照片重新演起。事件必须按“触发—反应—结果”的可见顺序发生，禁止先有结果后补原因。',
      ...
      const reviewDraft = async (
        ...
        const prompt = [
          ...
          'issues 最多返回 3 条，只保留高置信度且能指出正文证据的明显问题：人物未在场却行动或说话、说话人错配、道具归属前后矛盾、角色提前知道信息、因果或行动顺序矛盾、与前集状态直接冲突、前集或本集的同一动作被当成新事件重复发生。必须比较不相邻的前中后段以及连续性中的前集事件；若只是换了人物或措辞，却再次完整演“打开同一抽屉—拿取/查看同一照片或文件—收回”，报 REPEATED_ACTION；后段直接核验、转交或产生新后果不算重复。',
  ...
  private async generateDirectTextEpisode(input: {
    ...
    const previousEpisodesForRepeatCheck = state.episodes
      .filter((episode) => episode.episodeNumber < episodeNumber);
    const repeatedPlotIssues = detectRepeatedDirectPlotEvents(
      draftForReview,
