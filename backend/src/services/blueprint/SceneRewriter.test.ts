 ...
 *
 * Covers the局部重写 orchestration in isolation from the HTTP and provider
 * layers, using a REAL {@link FileDataStore} over a unique temp file plus a
 * FAKE {@link ModelProxy}:
 *
 * - An empty / whitespace-only `instruction` throws `VALIDATION_ERROR` before
 *   any provider call (Req 12.5).
 * - `MODEL_NOT_CONFIGURED` is thrown before any provider call when no model
 *   config is saved (the fake proxy records zero calls) (Req 12.7).
 * - A scene id not present in the blueprint throws `NOT_FOUND` (Req 12.4).
 * - A target scene with no persisted draft throws `VALIDATION_ERROR` (Req 12.6).
 * - Success path: `streamRewrite` returns `{ scene, stream }`; consuming the
 *   stream yields the canned deltas, and `finalizeDraft` then writes ONLY the
 *   target scene's draft, leaving other scenes' drafts unchanged (Req 12.3).
 *
...
import { SceneRewriter } from './SceneRewriter.js';
...
describe('SceneRewriter.streamRewrite', () => {
  let dir: string;
  let store: FileDataStore;
  let chapterId: string;

  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'scene-rewriter-'));
  ...
  it.each(['', '   ', '\n\t'])(
    ...
    async (instruction) => {
      ...
      const rewriter = new SceneRewriter(
        store,
      ...
      await expect(
        rewriter.streamRewrite(chapterId, 'scene-0', { instruction }, signal()),
      ).rejects.toSatisfy(
  ...
  it('throws MODEL_NOT_CONFIGURED before any provider call when no config is saved (Req 12.7)', async () => {
    ...
    await expect(
      rewriter.streamRewrite(chapterId, 'scene-0', { instruction: '让它更紧凑' }, signal()),
    ).rejects.toSatisfy(
  ...
  it('throws NOT_FOUND when the scene id is not in the blueprint (Req 12.4)', async () => {
    await store.saveModelConfig(VALID_CONFIG);
    const { proxy, calls } = makeFakeProxy();
    const rewriter = new SceneRewriter(
      store,
    ...
    await expect(
      rewriter.streamRewrite(chapterId, 'missing-scene', { instruction: '让它更紧凑' }, signal()),
    ).rejects.toSatisfy(
  ...
  it('throws VALIDATION_ERROR when the target scene has no draft (Req 12.6)', async () => {
    await store.saveModelConfig(VALID_CONFIG);
    const { proxy, calls } = makeFakeProxy();
    const rewriter = new SceneRewriter(
      store,
