...
- 每轮先由 Agent 输出可审计的自检清单：`confirmedFacts`（已确认事实）、`unresolvedDecisions`（仍会改变创作方向的决策）、`safeDefaults`（可由 Agent 自行补全的低风险细节）和 `hardConstraints`（不可违背的题材与限制）。问题必须从未决策项和当前题材动态生成；清单不是隐藏思维链。
...
- 单元测试必须证明问题来自模型决策，不得由服务端固定题库在模型调用前短路；模型返回有效问题时，服务端不得为了凑数量追加通用题目。
...
- 题材识别必须约束提问词汇：例如识别为校园后，问题围绕学校、学业、社团、竞赛、关系和校园秘密生成，不得出现魔法、修仙、骑士、王国等跨题材选项。
