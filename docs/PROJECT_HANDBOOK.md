...
设置页会显示实际请求地址：在 Base URL 去掉末尾斜杠后拼接 `/chat/completions`。是否需要 `/v1` 由服务商的 OpenAI 兼容入口决定；模型名称也必须使用该入口实际提供的 ID。

连接测试直接请求当前提供商，不读写本地响应缓存，只有收到非空白的最终正文才通过。HTTP 200 的网页、错误 JSON、流内错误、空流和只有推理的响应都算失败；合法 SSE 即使 Content-Type 不规范仍可识别。错误提示保留上游 HTTP 状态和脱敏原因。

配置已在浏览器保存不等于连接测试通过。连接测试通过说明该配置能返回正文；具体创作任务仍须通过其内容和结构校验。

### 写前依据与过期候选

小说和短剧正文面板可展开“写前依据”，查看本次目标、承接事项、禁项、作者要求及具体来源。生成前显示当前已保存资料的预览；生成后保留实际使用的依据。

如果生成期间修改了人物、大纲、前章或当前正文，迟到候选会被拒绝保存。先保留作者的新修改，再根据最新资料重新生成。普通手工编辑仍可正常保存，不能用手工保存入口替代 AI 候选的采用流程。接口与验证范围见 [P1 验收记录](WEBNOVEL_WRITER_P1_ACCEPTANCE.md)。
