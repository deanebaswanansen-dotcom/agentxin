...
以下契约适用于 AI 选题和短剧策划，手工保存仍使用原有编辑校验。

- `plan.turn` 的 `seedPrompt` 只传作者灵感；可选 `draft` 只包含可编辑策划字段。项目名称与类型由后端查询，已有草稿保存在问答会话中，不拼入灵感或故事字段的默认值。
- 模型须实际返回标题、梗概和核心冲突。空对象、无关对象、输入上下文回显及缺少核心故事字段不能靠旧稿或模板补成成功。保留字段别名、JSON 语法修复与非核心字段的本地补全。
- 每次结构化生成最多一次主调用、一次结构修复和一次显式配置的备用模型调用；这些阶段与传输重试共享最多四次实际 HTTP 请求。缓存命中不消耗 HTTP 额度，失败诊断记录实际尝试数。问答服务不再在外层重复整个生成过程。
- 失败返回明确原因，不保存或自动确认新策划；现有策划内容、状态和 revision 保留，失败 checkpoint 标记需要处理。不得返回固定模板并称其为 AI 生成的方案。
- 前端在异步结果返回、继续问答及自动确认前后检查项目、编辑与灵感版本。旧结果不覆盖新编辑，不清除 dirty；切项目后旧请求不能改动当前页面。已经发出的确认请求仍受后端 revision 校验，前端不能撤回已在服务端完成的请求。
- 后端按客户端和项目管理问答请求；重开会话取消旧生成，旧结果不能覆盖新会话，同会话并发续答返回冲突。检查点版本只在短写入队列内分配，不跨模型调用预占版本。
- 本轮不自动修正已经保存的污染内容，也不修改全剧大纲、人物或世界资料的生成策略；后续任务书与记忆阶段分别验收。

验收记录见 [P0 验收记录](WEBNOVEL_WRITER_P0_ACCEPTANCE.md)。

### 2026-09-15：写前任务书与来源版本

短剧正文生成确定性组装本集目标、承接事实、禁项、作者约束及来源摘录，并将同一份任务书用于生成、审查和界面。来源绑定策划、大纲/分集卡、人物集合、世界资料以及目标集之前的有效正文与连续性；新增或删除资料也会使旧候选失效。

提交复用原有正文与连续性原子边界，校验来源、目标正文 revision 和候选 hash；旧审查不得批准后来修改的正文。手工编辑继续独立校验。界面区分下次写作预览与实际生成依据，无来源的历史数据不补造已验证记录。接口和跨模式验收见 [P1 验收记录](WEBNOVEL_WRITER_P1_ACCEPTANCE.md)。
...
- 对话密度偏离策划值超过 15 个百分点。
- 可见字符少于目标的 75%，或超过目标的 125%。目标字数用于引导创作，不单独阻止成稿。
...
- 单集 1,200 字、最多 3 场时，正常路径目标为 3 次模型调用：场景计划、正文候选、连续性与明显错误审查。正文低于建议体量时最多追加一次自然续写；只有结构失败或可确定定位的人物、场景、道具、时间线 blocking issue 才使用 Fixup/Patch 预算。
- 单集异常路径的业务模型调用硬上限为 6 次；达到上限后保留结构合法的已有正文并进入人工检查，不允许重试层相乘或无限追求字数。
...
- 10 集测试全部生成，按 5 集分批，每集 1–5 场；目标字数的 75%—125% 作为质量提示区间，不因篇幅单独阻止成稿。
...
## 27. v1.1 产品化增量（2026-08-15 已确认）

v1.1 不替换 v1.0 的结构化领域模型和后台任务，而是在其上补齐成品阅读、批次导航、可见校稿和视觉资产入口。参考产品仅用于提炼功能与信息架构，不复制其付费锁、下载次数、具体文案、故事内容或视觉实现。

### 27.1 信息架构

短剧工作台左侧固定显示全剧完成集数、总集数和可见字符数，并依次提供：
...
正文批次按 `totalEpisodes` 动态派生，最后一批允许不足 5 集。批次状态为 `blocked | ready | generating | proofreading | completed | failed`；“锁定”只表示前置资料尚未成熟，不表示付费权限。

### 27.2 阅读与编辑双模式

- 策划、大纲、角色和世界页面默认使用成品阅读布局，编辑表单作为显式次级模式。
- 正文默认按 5 集连续排版，支持单集跳转、全屏、阅读/编辑切换、单集保存和校稿入口。
- 中文短剧排版统一使用“第 X 集、X-Y 地点 时间/内外、人物、字幕、动作、对白”的序列化结果；结构化 JSON 继续作为唯一事实源。
- 任何成品页都不得以一组裸表单代替可阅读的最终产物。
...
- 工作台首次加载使用一个聚合快照接口获取策划、总纲、角色、世界、单集摘要、批次摘要和校稿计数。
- 后台资料或正文任务完成后，前端必须重新获取受影响的资源，而不是只刷新任务卡和集数摘要。
- 成功响应必须清除同一请求产生的超时错误；项目切换必须取消旧项目请求。
- 空闲时停止 2 秒轮询，只在存在 `queued | running | retrying` 任务时继续。

### 27.5 导出可靠性

TXT、Markdown、DOCX 和 Fountain 均支持整本和当前批次。下载必须使用服务端或浏览器返回的真实文件名与 MIME 类型，延迟释放 Blob URL，并通过可打开文件验收；页面提示“已导出”但浏览器仍保留 `.crdownload` 不视为成功。

### 27.6 视觉资产边界

首步持久化封面提示词，并在界面明确展示未生成图片的占位状态；用户上传、独立视觉资产记录与图片模型适配器留作后续能力。图片能力不得阻塞策划、大纲、正文或导出；未配置图片模型时仍必须提供完整文字工作流。

### 27.7 v1.1 验收增量

- 60 集项目显示 12 个批次节点，完成数、总字数和批次状态准确。
- 1–5 集可在同一阅读页连续查看，排版与中文短剧序列化输出一致。
- 刷新、切换项目和服务重启后可恢复当前批次，已完成集不重复生成。
- 用户能查看、修复或忽略软校稿问题；未解决硬问题阻止完成。
- 四种格式下载最终落盘并带正确扩展名，不能残留未完成临时文件。
- 工作台资料加载完成后不得残留误导性的超时错误。
- 小说模式的加载、编辑、任务、阅读和导出全部回归通过。
